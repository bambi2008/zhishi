...
- Today 支持用户主动保存能量、压力、感受和关注领域；命理区域只显示已保存命盘的审计周期，没有命盘或审计失败时不生成个人判断
- “我现在有点乱”三步澄清流程只整理用户实际输入，用户确认后保存在本机，不生成替用户下结论的内容
- Journey 展示用户真实保存的每日状态趋势与事实梳理时间线，支持更新、全文搜索、感受筛选、详情回看、系统分享导出、逐条删除和统一清除，不伪造章节进度

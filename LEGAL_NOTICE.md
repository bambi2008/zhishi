...
8. “有点乱”中的现实反思对话与命盘 AI 解读是两条独立数据流。用户须在当次对话前明确确认，DeepSeek 只接收用户本次主动填写的事实、感受、解释、担心、问题及本轮补充消息，不附带命盘、出生资料、姓名、联系方式或账户标识。
9. 现实反思对话不得把模型输出包装成事实或替用户作重大决定。输出必须引用本次用户证据，先追问最高影响的缺失信息，再给出有适用条件和代价的不同选项、一个小步行动及核验问题，并明确假设与不确定性。
10. 现实反思对话与模型输出默认不保存到服务端或本机。仅当用户主动选择保存时，才在本机保存原始记录和最新一轮结构化综合；完整对话记录不保存。
...
- 现实反思对话可能包含关系、工作、压力或健康相关信息，应视为潜在敏感数据；不得要求用户提交姓名、联系方式、医疗记录、账号凭据或他人个人数据。
...
5. DeepSeek API 的 DPA、EU SCC/UK transfer mechanism、传输影响评估、命盘解释与现实反思两类 API 输入/输出的实际保留期限、训练/改进用途控制及删除协助。
6. 各类数据的实际保存期限与删除实现。
...
- 命盘 AI 与现实反思 AI 分别提供就地说明与明确确认；现实反思对话不自动附带命盘或出生资料，不自动保存完整对话。
- 现实反思模型输出通过证据引用、最新回复使用、假设性措辞、禁止高风险指令和关键安全信号停止机制的服务端校验。

### P1：付费上线前

- 购买页完成价格、周期、续费、取消、退款和 EU/UK 消费者权利展示。
- 完成美国州隐私请求流程、California Notice at Collection 和适用的选择退出信号。
- 完成命盘解释和现实反思两条 AI 流程的标识、提示词/输出安全评估，以及不记录请求正文前提下的可追溯事件与版本策略。

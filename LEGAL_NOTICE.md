...
7. “今天”和“今年”共用回答优先的 DeepSeek 对话。DeepSeek 只接收用户本轮主动提交的消息、可选的今天状态，以及服务端重新计算并审计后的最小命盘/周期事实包；不接收原始出生日期、地点名或经纬度。请求正文不得写日志，API Key 只保存在 Render 后端。
8. 输入框上方必须持续、就地说明本轮会发送的数据；用户点击发送构成本轮主动确认，不得用预勾选、模糊按钮或隐藏说明代替。不得附带姓名、联系方式、账户标识、密码、医疗记录、完整金融账户信息或设备上的其他记录。
9. 对话不得把模型输出包装成事实或替用户作重大决定。每轮必须先给明确但有条件的答案、一个带时间与完成标准的小步骤、2—3 个具体案例和风险提醒，再只问一个会改变建议的问题；用户补充后必须引用最新回复并重写完整建议。
10. 新对话与模型输出默认不保存到服务端或本机，关闭页面后清除。旧事实梳理及用户以前主动保存的结构化综合继续按既有本机保留、导出和删除机制管理；完整对话记录不保存。
...
- 统一指导对话可能包含关系、工作、压力或健康相关信息，应视为潜在敏感数据；不得要求用户提交姓名、联系方式、医疗记录、账号凭据、完整金融账户信息或他人个人数据。
...
5. DeepSeek API 的 DPA、EU SCC/UK transfer mechanism、传输影响评估、统一指导对话输入/输出的实际保留期限、训练/改进用途控制及删除协助。
6. 各类数据的实际保存期限与删除实现。
...
- 统一对话在输入框上方提供就地数据说明，并以每次点击发送作为主动确认；只附带可选今天状态和最小已审计命盘事实，不附带原始出生资料或其他设备记录。
- 对话输出通过计算/现实证据分层、最新回复使用、单一追问、具体行动与案例、禁止高风险及金融交易指令和关键安全信号停止机制的服务端校验。

### P1：付费上线前

- 购买页完成价格、周期、续费、取消、退款和 EU/UK 消费者权利展示。
- 完成美国州隐私请求流程、California Notice at Collection 和适用的选择退出信号。
- 完成统一回答优先对话的 AI 标识、提示词/输出安全评估，以及不记录请求正文前提下的可追溯事件与版本策略。

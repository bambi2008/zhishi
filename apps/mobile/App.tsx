...
function Kicker({ label, color = colors.sage }: { label: string; color?: string }) {
  return <View style={styles.kicker}><View style={[styles.kickerDot, { backgroundColor: color }]} /><Text style={[styles.kickerText, styles.kickerTextAccessible]}>{label}</Text></View>;
}
...
function Header({ onSafety, onProfile, apiOnline }: { onSafety: () => void; onProfile: () => void; apiOnline: boolean }) {
  return <View style={styles.header}>
    ...
    <View style={styles.headerRight}><Pressable accessibilityRole="button" accessibilityLabel="安全、隐私与使用条款" hitSlop={8} onPress={onSafety} style={[styles.headerIcon, styles.headerIconAccessible]}><Text>♡</Text></Pressable><Pressable accessibilityRole="button" accessibilityLabel="打开设置" hitSlop={6} onPress={onProfile} style={[styles.profileChip, styles.profileChipAccessible]}><View style={styles.avatar}><Text style={styles.avatarText}>我</Text></View><Text style={styles.profileName}>设置</Text><Text style={styles.chevron}>⌄</Text></Pressable></View>
  </View>;
}
...
type GuidanceActionsState = {
  loading: boolean;
  actions: GuidanceActionRecord[];
  error: string;
};

function useGuidanceActions(revision: number): GuidanceActionsState {
  const [state, setState] = useState<GuidanceActionsState>({ loading: true, actions: [], error: '' });
  useEffect(() => {
    let active = true;
    setState(previous => ({ ...previous, loading: true, error: '' }));
    loadGuidanceActions()
      .then(actions => {
        if (active) setState({ loading: false, actions, error: '' });
      })
      .catch(loadError => {
        if (!active) return;
        setState({ loading: false, actions: [], error: loadError instanceof Error ? loadError.message : '执行记录读取失败，请稍后重试。' });
      });
    return () => { active = false; };
  }, [revision]);
  return state;
...
function DailyScreen({ onGuidance, onBazi, onDailyState, todayState, dailyStateLoading, dailyStateError, revision }: { onGuidance: (prompt?: string) => void; onBazi: () => void; onDailyState: (record: DailyStateRecord | null) => void; todayState: DailyStateRecord | null; dailyStateLoading: boolean; dailyStateError: string; revision: number }) {
  ...
  return <View>
    ...
    {!loading && !context && !error ? <View style={[styles.card, styles.todayEmptyCard]}><Kicker label="命盘是可选的文化线索" color={colors.terracotta} /><Text style={styles.todayEmptyTitle}>没有命盘，也可以先聊现实问题。</Text><Text style={styles.todayEmptyText}>今天的对话会只依据你主动提供的现实信息；建立并保存命盘后，才会加入经过审计的文化周期线索。</Text><PrimaryButton label="可选：建立并保存命盘" onPress={onBazi} /></View> : null}
...
function DailyStateOverview({ records, loading, error, onOpen }: { records: DailyStateRecord[]; loading: boolean; error: string; onOpen: (record: DailyStateRecord | null) => void }) {
  ...
  const hasEnoughTrendData = recent.length >= 3;
  return <View style={[styles.card, styles.stateOverviewCard]}>
    <View style={styles.stateOverviewTop}><View style={styles.journeySummaryCopy}><Kicker label={`最近 ${recent.length} 次真实记录`} color={colors.blue} /><Text style={styles.stateOverviewTitle}>{hasEnoughTrendData ? '短期状态，不做诊断。' : '先积累记录，再看趋势。'}</Text></View><Text style={styles.stateOverviewCount}>{records.length}<Text style={styles.stateOverviewCountUnit}> 天</Text></Text></View>
    {hasEnoughTrendData ? <><View style={styles.stateAverageRow}><View style={styles.stateAverageItem}><Text style={styles.stateAverageLabel}>平均能量</Text><Text style={styles.stateAverageValue}>{averageEnergy.toFixed(1)}</Text></View><View style={styles.stateAverageDivider} /><View style={styles.stateAverageItem}><Text style={styles.stateAverageLabel}>平均压力</Text><Text style={styles.stateAverageValue}>{averageStress.toFixed(1)}</Text></View></View>
      ...
      <View style={styles.stateTrendLegend}><Text style={styles.stateTrendLegendEnergy}>● 能量</Text><Text style={styles.stateTrendLegendStress}>● 压力</Text><Text style={styles.stateTrendLimit}>只基于已保存数值</Text></View></> : <Text style={styles.stateTrendPending}>至少保存 3 天后再显示平均值和趋势，避免把单次状态误读成变化。</Text>}
...
const guidanceActionStatusLabels: Record<GuidanceActionRecord['status'], string> = {
  planned: '待执行',
  in_progress: '进行中',
  done: '已执行',
  skipped: '暂不做',
...
function GuidanceActionOverview({ actions, loading, error }: { actions: GuidanceActionRecord[]; loading: boolean; error: string }) {
  if (loading || error || !actions.length) return null;
  return <View style={[styles.card, styles.actionOverviewCard]}>
    <View style={styles.journeySummaryTop}><View style={styles.journeySummaryCopy}><Kicker label="建议 → 执行 → 新事实" color={colors.terracotta} /><Text style={styles.journeySummaryTitle}>最近的执行反馈</Text></View><Text style={styles.journeySummaryNumber}>{actions.length}</Text></View>
    <Text style={styles.actionOverviewText}>这些记录只保存在本机。下次和知时对话时，已填写的结果与新事实会作为现实证据带回讨论。</Text>
    {actions.slice(0, 3).map(action => <View key={action.id} style={styles.actionOverviewRow}><View style={styles.actionOverviewCopy}><Text style={styles.actionOverviewHeadline} numberOfLines={1}>{action.headline}</Text><Text style={styles.actionOverviewAction} numberOfLines={2}>{action.action}</Text>{action.newFact ? <Text style={styles.actionOverviewFact} numberOfLines={2}>新事实：{action.newFact}</Text> : null}</View><Text style={[styles.actionOverviewStatus, action.status === 'done' && styles.actionOverviewStatusDone]}>{guidanceActionStatusLabels[action.status]}</Text></View>)}
    {actions.length > 3 ? <Text style={styles.actionOverviewMore}>还有 {actions.length - 3} 条执行记录</Text> : null}
  </View>;
}

function JourneyScreen({ records, loading, error, dailyStates, dailyStatesLoading, dailyStatesError, guidanceActions, guidanceActionsLoading, guidanceActionsError, onGoToday, onOpenRecord, onOpenDailyState }: { records: ClarityRecord[]; loading: boolean; error: string; dailyStates: DailyStateRecord[]; dailyStatesLoading: boolean; dailyStatesError: string; guidanceActions: GuidanceActionRecord[]; guidanceActionsLoading: boolean; guidanceActionsError: string; onGoToday: () => void; onOpenRecord: (record: ClarityRecord) => void; onOpenDailyState: (record: DailyStateRecord | null) => void }) {
  ...
  return <View>
    ...
    <GuidanceActionOverview actions={guidanceActions} loading={guidanceActionsLoading} error={guidanceActionsError} />
...
function YearScreen({ onBazi, onGuidance, revision }: { onBazi: () => void; onGuidance: (prompt?: string) => void; revision: number }) {
  ...
  return <View>
    ...
    {!loading && context?.user_visible ? <View style={[styles.card, styles.yearGuidanceCard]}><Kicker label="年度对话" color={colors.gold} /><Text style={styles.yearGuidanceTitle}>把“有潜力但要稳健”问到可执行。</Text><Text style={styles.yearGuidanceText}>例如：该准备哪份文件、先验证什么、关系里怎样开口、投资决定先设置哪些边界。</Text><PrimaryButton label="和知时讨论今年" onPress={() => onGuidance('未来 90 天我最应该先推进什么？')} /></View> : null}
...
function DailyStateSheet({ record, onClose, onSaved, onDelete }: { record: DailyStateRecord | null; onClose: () => void; onSaved: (record: DailyStateRecord) => void; onDelete: (id: string) => Promise<void> }) {
  ...
  return <Sheet title={record ? '编辑每日状态' : '记录今天状态'} onClose={onClose}>
    ...
    <View accessibilityRole="radiogroup" accessibilityLabel="今天最接近的感受" style={styles.stateChoiceGrid}>{emotions.map(item => <Pressable key={item} accessibilityRole="radio" accessibilityState={{ checked: emotion === item }} aria-checked={emotion === item} onPress={() => setEmotion(item)} style={[styles.stateChoiceButton, emotion === item && styles.stateChoiceSelected]}><Text style={[styles.stateChoiceText, emotion === item && styles.stateChoiceTextSelected]}>{item}</Text></Pressable>)}</View>
    <Text style={styles.stateFieldLabel}>今天最关注的领域</Text>
    <View accessibilityRole="radiogroup" accessibilityLabel="今天最关注的领域" style={styles.stateFocusGrid}>{focusAreas.map(item => <Pressable key={item} accessibilityRole="radio" accessibilityState={{ checked: focusArea === item }} aria-checked={focusArea === item} onPress={() => setFocusArea(item)} style={[styles.stateFocusButton, focusArea === item && styles.stateChoiceSelected]}><Text style={[styles.stateChoiceText, focusArea === item && styles.stateChoiceTextSelected]}>{item}</Text></Pressable>)}</View>
...
function ProfileSheet({ onClose, onOpenBazi, recordCount, dailyStateCount, guidanceActionCount, onExportRecords, onClearRecords, onClearDailyStates, onClearGuidanceActions }: { onClose: () => void; onOpenBazi: () => void; recordCount: number; dailyStateCount: number; guidanceActionCount: number; onExportRecords: () => Promise<void>; onClearRecords: () => Promise<void>; onClearDailyStates: () => Promise<void>; onClearGuidanceActions: () => Promise<void> }) {
  ...
  const clearRecords = async (kind: 'clarity' | 'daily' | 'guidance') => {
    setClearing(true);
    setClearError('');
    try {
      if (kind === 'clarity') await onClearRecords();
      else if (kind === 'daily') await onClearDailyStates();
      else await onClearGuidanceActions();
    } catch (error) {
      setClearError(error instanceof Error ? error.message : '本机记录没有清除成功，请重试。');
      setClearing(false);
    }
  };
  const totalCount = recordCount + dailyStateCount + guidanceActionCount;
  return <Sheet title="设置与本机数据" onClose={onClose}>
    <Text style={styles.sheetEyebrow}>LOCAL FIRST · 本机优先</Text><Text style={styles.sheetTitle}>你保存什么，由你决定。</Text><Text style={styles.sheetSubtitle}>知时目前不要求注册账户。命盘、每日状态、事实梳理与建议执行反馈保存在当前设备，AI 生成内容仍不自动保存。</Text>
    ...
    <View style={styles.settingsDataCard}><Text style={styles.settingsDataLabel}>建议执行记录</Text><Text style={styles.settingsDataValue}>{guidanceActionCount} 条</Text><Text style={styles.settingsDataText}>记录建议是否执行、实际结果和新事实；下次对话会把已填写反馈作为现实证据。</Text></View>
    ...
    {guidanceActionCount > 0 && confirmingClear === null ? <Pressable disabled={Boolean(confirmingClear)} onPress={() => setConfirmingClear('guidance')} style={[styles.dangerTextButton, Boolean(confirmingClear) && styles.disabled]}><Text style={styles.dangerTextButtonLabel}>清除全部建议执行记录</Text></Pressable> : null}
    {confirmingClear === 'guidance' ? <View style={styles.deleteConfirmCard}><Text style={styles.deleteConfirmTitle}>清除全部 {guidanceActionCount} 条执行记录？</Text><Text style={styles.deleteConfirmText}>此操作不会删除事实梳理、每日状态或命盘；删除后无法恢复。</Text><View style={styles.deleteConfirmActions}><Pressable disabled={clearing} onPress={() => setConfirmingClear(null)} style={styles.deleteCancelButton}><Text style={styles.deleteCancelLabel}>取消</Text></Pressable><Pressable disabled={clearing} onPress={() => clearRecords('guidance')} style={[styles.deleteButton, clearing && styles.disabled]}><Text style={styles.deleteButtonLabel}>{clearing ? '正在清除…' : '确认全部清除'}</Text></Pressable></View></View> : null}
...
function TabItem({ active, label, icon, onPress }: { active: boolean; label: string; icon: React.ComponentProps<typeof SymbolView>['name']; onPress: () => void }) {
  const tintColor = active ? colors.ink : '#5F655F';
  return <Pressable accessibilityRole="button" accessibilityLabel={label} accessibilityState={{ selected: active }} onPress={onPress} style={styles.tabItem}><SymbolView name={icon} size={23} tintColor={tintColor} weight={active ? 'semibold' : 'regular'} /><Text style={[styles.tabLabel, styles.tabLabelAccessible, active && styles.tabActive]}>{label}</Text></Pressable>;
}
...
export default function App() {
  ...
  const [guidanceActionRevision, setGuidanceActionRevision] = useState(0);
  ...
  const guidanceActionsState = useGuidanceActions(guidanceActionRevision);
  ...
  const handleClearGuidanceActions = async () => {
    await clearGuidanceActions();
    setGuidanceActionRevision(value => value + 1);
    setModal(null);
  };
  const handleExportRecords = async () => {
    ...
    const guidanceActionSections = guidanceActionsState.actions.map((action, index) => {
      const fields = [
        `${index + 1}. ${action.scope === 'today' ? '今天' : '今年'} · ${formatClarityDate(action.createdAt, true)}`,
        `建议：${action.action}`,
        `时间窗口：${action.when}`,
        `完成标准：${action.doneWhen}`,
        `状态：${guidanceActionStatusLabels[action.status]}`,
        action.completedAt ? `完成时间：${new Date(action.completedAt).toLocaleString('zh-CN')}` : '',
        action.outcome ? `结果：${action.outcome}` : '',
        action.newFact ? `新事实：${action.newFact}` : '',
      ].filter(Boolean);
      return fields.join('\n');
    });
    const messageSections = [
      ...
      guidanceActionSections.length ? `【建议执行记录】\n${guidanceActionSections.join('\n\n')}` : '',
    ].filter(Boolean);
    await Share.share({
      title: '知时 · 本机现实记录',
  ...
  return <SafeAreaView style={styles.safeArea}><StatusBar barStyle="dark-content" backgroundColor={colors.paper} /><View style={styles.app}><Header onSafety={() => setModal('safety')} onProfile={() => { setProfileSession(value => value + 1); setModal('profile'); }} apiOnline={apiOnline} /><ScrollView ref={mainScrollRef} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'} keyboardShouldPersistTaps="always" automaticallyAdjustKeyboardInsets>{view === 'daily' ? <DailyScreen onGuidance={prompt => openGuidance('today', prompt)} onBazi={() => setModal('bazi')} onDailyState={openDailyState} todayState={todayState} dailyStateLoading={dailyStateState.loading} dailyStateError={dailyStateState.error} revision={chartRevision} /> : view === 'journey' ? <JourneyScreen records={clarityState.records} loading={clarityState.loading} error={clarityState.error} dailyStates={dailyStateState.records} dailyStatesLoading={dailyStateState.loading} dailyStatesError={dailyStateState.error} guidanceActions={guidanceActionsState.actions} guidanceActionsLoading={guidanceActionsState.loading} guidanceActionsError={guidanceActionsState.error} onGoToday={() => setView('daily')} onOpenRecord={openRecord} onOpenDailyState={openDailyState} /> : <YearScreen onBazi={() => setModal('bazi')} onGuidance={prompt => openGuidance('year', prompt)} revision={chartRevision} />}</ScrollView><TabBar view={view} setView={setView} onBazi={() => setModal('bazi')} />
    ...
    <Modal visible={modal === 'profile'} transparent animationType="slide" onRequestClose={() => setModal(null)}><View style={styles.modalBackdrop}><ProfileSheet key={`profile-${profileSession}-${clarityRevision}-${dailyStateRevision}-${guidanceActionRevision}`} onClose={() => setModal(null)} onOpenBazi={() => setModal('bazi')} recordCount={clarityState.records.length} dailyStateCount={dailyStateState.records.length} guidanceActionCount={guidanceActionsState.actions.length} onExportRecords={handleExportRecords} onClearRecords={handleClearRecords} onClearDailyStates={handleClearDailyStates} onClearGuidanceActions={handleClearGuidanceActions} /></View></Modal>
...
const styles = StyleSheet.create({
  inputVoiceHint: { color: '#626862', fontSize: 10, lineHeight: 16, marginTop: 6 },
  ...
  actionOverviewCard: { backgroundColor: '#FAF2E8', marginTop: 13 }, actionOverviewText: { color: colors.muted, fontSize: 10, lineHeight: 17, marginTop: 10 }, actionOverviewRow: { borderTopWidth: 1, borderTopColor: 'rgba(169,87,56,0.16)', paddingVertical: 11, flexDirection: 'row', alignItems: 'flex-start', gap: 10 }, actionOverviewCopy: { flex: 1 }, actionOverviewHeadline: { color: colors.ink, fontSize: 11, fontWeight: '700' }, actionOverviewAction: { color: colors.muted, fontSize: 10, lineHeight: 16, marginTop: 4 }, actionOverviewFact: { color: colors.terracotta, fontSize: 9, lineHeight: 15, marginTop: 4 }, actionOverviewStatus: { color: colors.terracotta, backgroundColor: '#F3DFD5', borderRadius: 99, paddingHorizontal: 8, paddingVertical: 5, fontSize: 8, fontWeight: '700' }, actionOverviewStatusDone: { color: colors.sageDeep, backgroundColor: '#E2EBDD' }, actionOverviewMore: { color: colors.terracotta, fontSize: 9, textAlign: 'right', marginTop: 2 },
  ...
  headerIconAccessible: { width: 44, height: 44, alignItems: 'center', justifyContent: 'center', padding: 0 },
  profileChipAccessible: { minHeight: 44, paddingHorizontal: 4 },
  kickerTextAccessible: { color: '#646A63', fontSize: 10 },
  tabLabelAccessible: { color: '#5F655F' },
  stateTrendPending: { color: colors.muted, fontSize: 11, lineHeight: 18, marginTop: 16, padding: 13, borderRadius: 12, backgroundColor: '#FFFDF8' },
});

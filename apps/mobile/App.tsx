...
function SafetySheet({ onClose }: { onClose: () => void }) {
  return <Sheet title="安全与使用条款" onClose={onClose}><View style={styles.safetyIcon}><Text>♡</Text></View><Text style={styles.sheetTitle}>现实优先，命理只是文化视角。</Text><Text style={styles.sheetSubtitle}>知时不会替代医疗、法律、财务或心理服务，也不会用“注定”“必然”制造恐惧。涉及高风险内容时，系统会优先建议你联系专业人士或可信任的人。</Text>{['不预测灾祸、生死和疾病', '不替你做重大人生决定', '所有判断都显示信息来源与限制'].map(item => <Text key={item} style={styles.safetyItem}>✓  {item}</Text>)}<View style={styles.legalNoticeCard}><Text style={styles.legalNoticeTitle}>出生资料与计算责任提示</Text><Text style={styles.legalNoticeText}>排盘依据用户主动提交的出生日期、时间、地点和规则选项生成。用户选择非精确时间后，知时仍按其填写的具体时刻正常计算。</Text><Text style={styles.legalNoticeText}>实际出生时刻与填写值存在偏差时，四柱、大运、流年、流月及相关解释可能变化。用户应尽量核对资料，并理解由输入偏差导致的结果差异不等同于同一输入下的计算错误。</Text><Text style={styles.legalNoticeText}>上述提示不排除或限制知时依法不得排除或限制的责任。正式上线前，完整条款和隐私规则应由运营地区执业律师复核。</Text></View><PrimaryButton label="我知道了" onPress={onClose} /></Sheet>;
}

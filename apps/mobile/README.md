...
“我现在有点乱”会把用户填写的事实、感受、解释、担心和下一步问题分层展示。只有用户点击“保存这次梳理”后才通过 AsyncStorage 写入当前设备；保存后可在“章节”按关键词搜索、按感受筛选、继续编辑或逐条删除，也可从设置调用系统分享导出全部文字或统一清除。修改会保留原创建时间并单独更新时间；记录不会自动发送给知时 API 或 DeepSeek。
...
出生资料提交前会显示美国与欧洲隐私基线提示：当前 MVP 的计算 API 不主动持久化请求，不销售数据、不投放行为广告；命盘、每日状态和事实梳理仅在用户主动点击保存后写入本机。AI 解读按钮另有即时披露，说明最小化事实包和可选问题将发送给 DeepSeek；现实记录不会自动发送。完整欧美上线草案见根目录 `TERMS_OF_USE.md`、`PRIVACY_NOTICE.md` 和内部 `LEGAL_NOTICE.md`。

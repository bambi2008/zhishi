...
出生资料提交前会显示美国与欧洲隐私基线提示：当前 MVP 的计算 API 不主动持久化请求，不销售数据、不投放行为广告；仅在用户主动点击“保存”后写入本机。AI 解读按钮另有即时披露，说明最小化事实包和可选问题将发送给 DeepSeek。完整欧美上线草案见根目录 `TERMS_OF_USE.md`、`PRIVACY_NOTICE.md` 和内部 `LEGAL_NOTICE.md`。
